...
/**
 * MooAgents Engine Helper for JsHelper
 *
 * Provides MooAgents specific JavaScript for JsHelper.
 * Assumes that you have the following MooAgents packages
 *
 ...
 * @package       Cake.View.Helper
 */
class MooagentsEngineHelper extends JsBaseEngineHelper {

/**
 * Option mappings for MooAgents
 *
 * @var array
 */
 ...
 *
 * Requires the `Sortables` plugin from MooagentsMore
 *
 ...
 *
 * Requires the `Drag` plugin from MooagentsMore
 *
 ...
 *
 * Requires the `Drag` and `Drag.Move` plugins from MooagentsMore
 *
 * Droppables in Mooagents function differently from other libraries. Droppables
 ...
 * Mooagents droppables inherit all options from Drag.
 *
 ...
 */
	public function drop($options = array()) {
		if (empty($options['drag'])) {
			trigger_error(
				__d('cake_dev', '%s requires a "drag" option to properly function'), 'MooagentsEngine::drop()', E_USER_WARNING
			);
			return false;
		}
 ...
 *
 * Requires `Slider` from MooagentsMore
 *
